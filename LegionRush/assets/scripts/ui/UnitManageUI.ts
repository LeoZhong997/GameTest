...
const CARD_W = 140, CARD_H = 180;
const CARD_GAP = 16;
const DETAIL_W = 360, DETAIL_H = 440;
...
export class UnitManageUI extends Component {
    ...
    private buildListArea(parent: Node): void {
        ...
        const totalTabW = RACE_ORDER.length * TAB_W + (RACE_ORDER.length - 1) * 10;
        ...
        RACE_ORDER.forEach((race, i) => {
            ...
            tab.setPosition(tabStartX + i * (TAB_W + 10), 0, 0);
            ...
            this.addLabel(tab, RACE_NAMES[race] || race, 14, WHITE, 0, 0, TAB_W, true);
        ...
        lcut.setContentSize(520, SH - 80 - TAB_H - 20);
        lcut.setAnchorPoint(0.5, 0.5);
        this._listContent.setPosition(0, -40, 0);
    ...
    private refreshTabStyles(tabNode: Node): void {
        for (const tab of tabNode.children) {
            ...
            g.clear();
            g.fillColor = active ? new Color(30, 80, 140, 255) : CARD_BG;
            g.roundRect(-40, -15, 80, 30, 6);
            g.fill();
            g.strokeColor = active ? SEL_BORDER : CARD_BORDER;
    ...
    private buildDetailPanel(parent: Node): void {
        ...
        panel.setPosition(SW / 2 - DETAIL_W / 2 - 20, -20, 0);
    ...
    private refreshUnitList(): void {
        ...
        this._playerUnits.forEach((unit, i) => {
            ...
            cardGfx.fillColor = isSelected ? new Color(30, 70, 120, 255) : CARD_BG;
            cardGfx.roundRect(-CARD_W / 2, -CARD_H / 2, CARD_W, CARD_H, 8);
            cardGfx.fill();
            cardGfx.strokeColor = isSelected ? SEL_BORDER : CARD_BORDER;
            ...
            // Race color strip
            const stripNode = new Node('Strip');
            ...
            strg.fillColor = RACE_COLORS[cfg.race] || new Color(74, 144, 217, 255);
            ...
            sut.setContentSize(32, 32);
            sut.setAnchorPoint(0.5, 0.5);
            shapeNode.setPosition(0, 40, 0);
            const sg = shapeNode.addComponent(Graphics);
            sg.fillColor = RACE_COLORS[cfg.race] || new Color(74, 144, 217, 255);
            drawShape(sg, shape, 32);
            sg.fill();
            card.addChild(shapeNode);

            // Name
            this.addLabel(card, cfg.name, 14, WHITE, 0, 10, CARD_W, true);

            // Level + Quality
            const qColor = QUALITY_COLORS[unit.quality] || GRAY_TEXT;
            this.addLabel(card, `Lv${unit.level}`, 12, GRAY_TEXT, -20, -10, 60, true);
            this.addLabel(card, QUALITY_NAMES[unit.quality] || unit.quality, 12, qColor, 40, -10, 40, true);

            // Deploy count
            const deployCount = LevelSystem.instance.getDeployCount(unit.level);
            this.addLabel(card, `上阵 ×${deployCount}`, 11, GOLD, 0, -30, CARD_W, true);
    ...
    private refreshDetail(): void {
        ...
        y -= 55;
        this.addLabel(this._detailPanel, '--- 属性 ---', 12, GRAY_TEXT, 0, y, DETAIL_W, true);
        y -= 18;
        ...
        this.addLabel(this._detailPanel,
            `HP:${actualStats.hp}  攻:${actualStats.atk}  防:${actualStats.def}  速:${actualStats.spd}`,
            12, GRAY_TEXT, 0, y, DETAIL_W - 20, true);
        y -= 16;
        this.addLabel(this._detailPanel, `成长系数: ×${mult.toFixed(1)}`, 11,
            new Color(150, 130, 200, 255), 0, y, DETAIL_W, true);

        // ---- EXP progress bar ----
        y -= 24;
        this.addLabel(this._detailPanel, '经验进度', 13, GRAY_TEXT, 0, y, DETAIL_W, true);

        const BAR_W = 280, BAR_H = 14;
        const barY = y - 12;
        ...
        bbg.roundRect(-BAR_W / 2, -BAR_H / 2, BAR_W, BAR_H, 4);
        bbg.fill();
        ...
        bfg.rect(-BAR_W / 2, -BAR_H / 2, fillW, BAR_H);
        bfg.fill();
        ...
        this.addLabel(this._detailPanel, expText, 11, GRAY_TEXT, 0, y, DETAIL_W, true);
        ...
        dvGfx.fillColor = new Color(80, 80, 120, 80);
        ...
        const TAB_BAR_H = 36;
        const tabBarY = -DETAIL_H / 2 + TAB_BAR_H / 2 + 10;
    ...
    private buildUpgradeContent(unit: UnitInstanceData, topY: number, _bottomY: number): void {
        ...
        this.addLabel(this._detailPanel, '使用经验书', 14, WHITE, 0, y, DETAIL_W, true);
        y -= 28;

        const bookIds = LevelSystem.instance.getExpBookIds();
        const bookStartX = -(bookIds.length * (BTN_W + 10)) / 2 + BTN_W / 2;
        ...
        for (let i = 0; i < bookIds.length; i++) {
            ...
            const bx = bookStartX + i * (BTN_W + 10);

            const bookBtn = new Node(`BookBtn_${bookId}`);
            const but = bookBtn.addComponent(UITransform);
            but.setContentSize(BTN_W, BTN_H + 10);
            but.setAnchorPoint(0.5, 0.5);
            bookBtn.setPosition(bx, y, 0);

            const btnBg = new Node('BookBtnBg');
            const btnbgut = btnBg.addComponent(UITransform);
            btnbgut.setContentSize(BTN_W, BTN_H + 10);
            btnbgut.setAnchorPoint(0.5, 0.5);
            btnBg.setPosition(0, 0, 0);
            const btnbg = btnBg.addComponent(Graphics);
            btnbg.fillColor = owned > 0 ? new Color(30, 80, 140, 255) : new Color(40, 40, 55, 255);
            btnbg.roundRect(-BTN_W / 2, -(BTN_H + 10) / 2, BTN_W, BTN_H + 10, BTN_R);
            btnbg.fill();
            btnbg.strokeColor = owned > 0 ? CARD_BORDER : new Color(60, 60, 80, 150);
            btnbg.lineWidth = 1;
            btnbg.roundRect(-BTN_W / 2, -(BTN_H + 10) / 2, BTN_W, BTN_H + 10, BTN_R);
            btnbg.stroke();
            bookBtn.insertChild(btnBg, 0);

            this.addLabel(bookBtn, bookCfg.name, 13, WHITE, 0, 6, BTN_W, true);
            this.addLabel(bookBtn, `+${bookCfg.exp}EXP`, 11, GOLD, 0, -6, BTN_W, true);
            this.addLabel(bookBtn, `库存:${owned}`, 10, owned > 0 ? GRAY_TEXT : DIM, 0, -18, BTN_W, true);
    ...
    private buildAscendContent(unit: UnitInstanceData, topY: number, _bottomY: number): void {
        ...
        // 碎片持有量
        const shardCounts = UpgradeSystem.instance.getShardCounts(configId, inv);
        this.addLabel(this._detailPanel, '碎片持有', 14, WHITE, 0, y, DETAIL_W, true);
        y -= 24;
        for (const sc of shardCounts) {
            ...
            this.addLabel(this._detailPanel, `${qName}碎片: ${sc.count}`, 13, qColor, 0, y, DETAIL_W, true);
            y -= 20;
        }
        ...
        y -= 8;
        for (const synth of synthesisList) {
            ...
            this.addLabel(synthBtn, `${synth.fromName}碎片 ×${count} → ${synth.toName}碎片 ×1`, 13,
                check.can ? WHITE : DIM, 0, 0, 260, true);
        ...
        // 品质跃迁
        this.addLabel(this._detailPanel, `${qName}`, 15, qColor, -60, y, 60, true);
        this.addLabel(this._detailPanel, '→', 16, WHITE, 0, y, 30, true);
        this.addLabel(this._detailPanel, nextQName, 15, nextQColor, 60, y, 60, true);

        // 升阶消耗
        const cost = us.getCost(unit)!;
        const shardHave = inv[cost.materialId] || 0;
        y -= 22;
        this.addLabel(this._detailPanel, `消耗: ${qName}碎片 ${shardHave}/${cost.count}`, 12,
            shardHave >= cost.count ? GRAY_TEXT : new Color(255, 100, 100, 255), 0, y, DETAIL_W, true);
        ...
        const btnText = check.can ? '升  阶' : (check.reason.length > 8 ? check.reason.substring(0, 7) + '…' : check.reason);
        this.addLabel(upBtn, btnText, 14,
            check.can ? WHITE : new Color(180, 160, 200, 255), 0, 0, 160, true);
